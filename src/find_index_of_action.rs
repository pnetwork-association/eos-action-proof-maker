...
pub fn find_index_of_action_and_put_in_state(state: State) -> Result<State> {
    ...
    get_index_of_action(
        state.get_eos_action()?,
        state.get_eos_action_receipts()?,
    )
...
mod tests {
    ...
    fn should_get_digest_from_action() {
        let expected_result =
            "364afa1cc13bca5dce1027f089e56889171373f66f5e3e59637251aaaeac4caa"
            .to_string();
        let action = get_sample_action_n(1)
            .unwrap();
        let result = get_digest_from_action(&action)
            .unwrap();
    ...
    fn should_get_index_of_action() {
        ...
        let action_receipts = get_sample_action_receipts_n(sample_num)
            .unwrap();
        let action = get_sample_action_n(sample_num)
            .unwrap();
        let result = get_index_of_action(
            &action,
            &action_receipts,
        ).unwrap();
